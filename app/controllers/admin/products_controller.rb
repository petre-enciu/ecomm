class Admin::ProductsController < AdminController
  ...
  def index
    if params[:query].present?
      @products_pagy, @admin_products = pagy(Product.where("name LIKE ?", "%#{params[:query]}%"))
    else
      @products_pagy, @admin_products = pagy(Product.all)
    end
  end
  ...
  def update
    ...
    if @admin_product.update(admin_product_params.reject{|k| k["images"]})
      ...
      redirect_to admin_products_path, notice: "Product updated sucessfully"
    else
